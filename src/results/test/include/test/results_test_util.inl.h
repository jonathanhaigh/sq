...
void add_fields_to_obj_data(ObjData &obj, std::string_view field_name,
  ...
  add_fields_to_obj_data(obj, field_name,
                         ResultTree{to_primitive(SQ_FWD(field_data))});
}

void add_fields_to_obj_data(ObjData &obj, std::string_view field_name,
                            ResultTreeDataAlternative auto &&field_data) {
  add_fields_to_obj_data(obj, field_name, ResultTree{SQ_FWD(field_data)});
}

void add_fields_to_obj_data(ObjData &obj, std::string_view field_name,
                            auto &&field_data, auto &&...args) {
...
void add_items_to_array_data(ArrayData &arr, PrimitiveLike auto &&field_data) {
  add_items_to_array_data(arr, ResultTree{to_primitive(SQ_FWD(field_data))});
}

void add_items_to_array_data(ArrayData &arr,
                             ResultTreeDataAlternative auto &&field_data) {
